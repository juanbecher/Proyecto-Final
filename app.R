...
# Proyeccion$Años <- round(Proyeccion$Años, digits = 2)
# # CARGA
# #Altas <- getAltas()
...
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem(tabName = "DemoraBeneficios",
            ...
            fluidRow(
              column(6,
                     actionButton(inputId = "JO", icon = icon("user-circle", class="user-clock"), width = '100%',style = "font-size: 48px; color: Dodgerblue;" ,h2("Jub-Ordinaria"))
              ),
              column(6,
                     actionButton(inputId = "JI", icon = icon("user-circle"), width = '100%',style = "font-size: 48px; color: green;" , h2("Jub-Invalidez"))
              )
    ...
    tabItem(tabName = "ComisionesActLaboral",
            ...
            wellPanel(
              tabsetPanel(
                ...
                tabPanel("Tabla",
                         DT::dataTableOutput("tablaActLaboral"))
              )
...
server <- function(input, output, session) {
  ...
  output$graficoProdu <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_xAxis(type="category") %>%
      hc_add_series(data = data, name = "Productividad", type = input$typeProdu, hcaes(x = mesAnio , y = Prom), color = c("#68BAF3") ) %>%
  ...
  output$tablaACA <- DT::renderDataTable({
    ...
    tabla <- rh_ACA[year(rh_ACA$FECHA) >= input$IAnioDesdeACA & year(rh_ACA$FECHA) <= input$IAnioHastaACA, ]
    DT::datatable(tabla)})
  #
  output$tablaActLaboral <- DT::renderDataTable({
    
    
    DT::datatable(rh_comision)})
  #
  ...
  #####################d
  output$tablaProyeccionAltas <- DT::renderDataTable(Proyeccion)
